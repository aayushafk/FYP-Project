...
const EventDetails = () => {
  ...
  const { joinEventRoom, leaveEventRoom, updateStatus } = useStatus();
  ...
  useEffect(() => {
    if (currentUser && eventId) {
      ...
      joinEventRoom(eventId, currentUser._id, currentUser.fullName, currentUser.role);

      return () => {
        // Leave the event room when component unmounts
        leaveEventRoom(eventId);
      };
    }
  }, [currentUser, eventId, joinEventRoom, leaveEventRoom]);
  ...
  const handleStatusUpdate = async (statusData) => {
    try {
      ...
      const response = await api.patch(`/volunteer/events/${eventId}/status`, {
        newStatus: statusData.newStatus
      });

      if (response.data) {
        // Update event state to reflect new status
        setEvent(prevEvent => ({
          ...prevEvent,
          trackingStatus: response.data.event.trackingStatus,
          volunteerAssignments: response.data.event.volunteerAssignments
        }));
      }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {event && (
          <div className="bg-white rounded-lg shadow-md overflow-hidden mb-6">
            <StatusTracker
              eventId={eventId}
              currentStatus={event.trackingStatus || 'Pending'}
              volunteerStatus={
                event.volunteerAssignments?.find(
                  va => va.volunteerId?._id === currentUser?._id || va.volunteerId === currentUser?._id
                )?.status
              }
              assignedVolunteers={event.assignedVolunteers || []}
              currentUserId={currentUser?._id}
              onStatusUpdate={handleStatusUpdate}
              onLoadingChange={setStatusLoading}
            />
          </div>
        )}

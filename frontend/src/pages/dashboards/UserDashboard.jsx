...
const UserDashboard = () => {
    ...
    const getRequestDetailRoute = (requestId) => `/dashboard/user/help-request/${requestId}`;

    const renderRecommendationPreview = (request) => {
        const recommendations = Array.isArray(request.aiRecommendedVolunteers)
            ? request.aiRecommendedVolunteers.slice(0, 3)
            : [];

        return (
            <div className="mt-3 p-4 bg-white rounded-xl shadow-md hover:shadow-lg transition-all border border-gray-100">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-500 mb-2">Recommended Volunteers</p>
                {recommendations.length === 0 ? (
                    <p className="text-sm text-gray-500">No recommendations yet</p>
                ) : (
                    <div className="space-y-2">
                        {recommendations.map((volunteer, index) => {
                            const volunteerSkills = Array.isArray(volunteer.skills) ? volunteer.skills.slice(0, 2) : [];
                            return (
                                <div key={volunteer.volunteerId || `${request._id}-vol-${index}`} className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                        <p className="text-sm font-semibold text-gray-800 truncate">{volunteer.fullName || 'Volunteer'}</p>
                                        <p className="text-xs text-gray-500 truncate">
                                            Skill: {volunteerSkills.length > 0 ? volunteerSkills.join(', ') : 'General Support'}
                                        </p>
                                    </div>
                                    <span className="text-xs font-semibold text-blue-700 bg-blue-100 border border-blue-200 px-2 py-0.5 rounded-full whitespace-nowrap">
                                        {volunteer.scorePercentage || `${Math.round(volunteer.score || 0)}%`}
                                    </span>
                                </div>
                            );
                        })}
                                    ...
                                    <div className="space-y-2">
                                        {myRequests.slice(0, 4).map(request => {
                                            ...
                                            return (
                                                <div key={request._id} className={`flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 transition-all cursor-pointer group ${
                                                ...
                                                }`} onClick={() => navigate(getRequestDetailRoute(request._id))}>
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        {renderRecommendationPreview(request)}
                                                    </div>
                                    ...
                                    {myRequests.map(request => {
                                        ...
                                        return (
                                            <div key={request._id} className={`flex items-center gap-4 p-4 rounded-xl hover:bg-slate-50 transition-all group border hover:border-gray-200 hover:shadow-sm ${
                                                ...
                                                <div className="flex-1 min-w-0 cursor-pointer" onClick={() => navigate(getRequestDetailRoute(request._id))}>
                                                    <div className="flex items-center gap-2 mb-1">

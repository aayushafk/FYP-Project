...
const EventDetailPage = () => {
  ...
  const getComparableId = (value) => {
    ...
    return candidate?.toString ? candidate.toString() : String(candidate);
  };
  ...
  useEffect(() => {
    ...
    const handleStatusUpdate = (data) => {
      ...
      if (getComparableId(data.eventId) === getComparableId(eventId)) {
        ...
        setEvent(prev => {
          ...
          if (data.volunteerId && updatedEvent.volunteerAssignments) {
            updatedEvent.volunteerAssignments = updatedEvent.volunteerAssignments.map(assignment => {
              const assignmentVolunteerId = getComparableId(assignment.volunteerId);
              if (assignmentVolunteerId === getComparableId(data.volunteerId)) {
        ...
        // Only show toast for other volunteers, not for self
        if (!isCurrentUser) {
        ...
        // Keep UI in sync with server truth after every status update event.
        fetchEventDetails();
      }
    };
  ...
  const isVolunteerAssigned = () => {
    const currentUserId = getComparableId(user?._id);
    return Boolean(event?.assignedVolunteers?.some(v => getComparableId(v) === currentUserId));
  };

  const getVolunteerStatus = () => {
    if (!event?.volunteerAssignments || !user?._id) return null;
    
    const userIdStr = getComparableId(user._id);
    const assignment = event.volunteerAssignments.find(a => {
      const volunteerIdStr = getComparableId(a.volunteerId);
  ...
  const getVolunteerParticipationStatus = () => {
    ...
    const userIdStr = getComparableId(user._id);
    const assignment = event.volunteerAssignments.find(a => getComparableId(a.volunteerId) === userIdStr);
    ...
    return (
      <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6 mb-6">
        ...
        {canUpdate && displayStatus !== 'Completed' && (
          ...
          {updatingStatus && (
            <div className="flex gap-2 justify-center pt-4 text-sm text-indigo-600">
                      ...
                      >
                        ...
                        >
                          <div className={`font-semibold text-xs mb-1 ${
                            isOwnMessage(msg) ? 'text-blue-100' : 'text-gray-600'
                          }`}>
                            {isOwnMessage(msg)
                              ? 'You' 
                              : msg.sender?.role 
